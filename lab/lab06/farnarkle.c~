...
int count_arkles(int tiles1[N_TILES], int tiles2[N_TILES]) {
    int i=0,j=0, output1=0, output2=0,output=0;
    while(i<N_TILES){
        j=0;
        while(j<N_TILES){
    ...
    while(i<N_TILES){
        j=0;
        while(j<N_TILES){
            if(tiles2[i]==tiles1[j] && tiles2[i]!=tiles1[i]){
                output2=output2+1;
                j=N_TILES;
            }
            j=j+1;
...
void play_farnarkle(int hidden_tiles[N_TILES]) {
    //FIND WHETHER PLAYER WINS
    int win=0;  
    int num_of_turn=1,i=0;                                                //initalize it is false at first
    while(win!=1){
        i=0;
        int input_tiles[N_TILES]={ };
        printf("Enter guess for turn %d: ",num_of_turn);
        while(i<N_TILES){
            scanf("%d",&input_tiles[i]);
            i=i+1;
        }
        
        int arkle=count_arkles(input_tiles, hidden_tiles);
        //FIND THE FARNARKLE
        int farnarkle=count_farnarkles(input_tiles, hidden_tiles);
        //PRINT THE NEXT LINE
        printf("%d farnarkles, %d arkles\n",farnarkle,arkle);
        if(farnarkle!=N_TILES){
            num_of_turn++;
        }else{
            printf("You win\n");
            win=1;
...
void farnarkle_ai(int turn, int previous_guesses[MAX_TURNS][N_TILES],
    ...
    //printf("turn 1= %d",turn);
    //printf("previous_guesses[%d][%d]=%d\n",turn,i,previous_guesses[turn][i]);
    printf("Number of turn: %d\n",turn);
    if(turn==1){
        while(i<N_TILES){
            guess[i]=1;
            previous_guesses[0][i]=guess[i];
            i=i+1;
        }
    }else if(turn!=1){
        i=0;
        //printf("previous_guesses[%d][%d]=%d\n",turn-2,i,previous_guesses[turn-2][i]);
        if(farnarkles==0){
            while(i<N_TILES){
                guess[i]=previous_guesses[turn-2][i]+1;
                previous_guesses[turn][i]=guess[i];
                i=i+1;
            } 
        }else if(farnarkles=1){
            while(i<N_TILES){
                guess[i]=previous_guesses[turn-2][i]+1;
                previous_guesses[turn][i]=guess[i];
                i=i+1;
