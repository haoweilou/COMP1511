...
void species_count(char species[], int n_sightings, struct pod sightings[n_sightings], int *n_pods, int *n_whales) {
    int i = 0;
    int num_of_sightings = 0, number_of_whales = 0;
    while(i < n_sightings){
        if(strcmp (species,sightings[i].species)==0){
            num_of_sightings++;
            number_of_whales = number_of_whales + sightings[i].how_many;
        }
        i++;
    }
    *n_pods = num_of_sightings; // CHANGE ME
    *n_whales = number_of_whales; // CHANGE ME
}
